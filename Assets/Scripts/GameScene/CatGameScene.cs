...
public class CatGameScene : MonoBehaviour
{
    ...
    private void Start()
    {
        preview = BackendGameData.userData == null;
        data = preview ? BackendUserData.CreateNew() : BackendGameData.userData;
        if (catalog == null) { catalog = ScriptableObject.CreateInstance<CatGameCatalog>(); ownCatalog = true; }
        ...
        Button(header, "저장", new Vector2(.73f,.53f), new Vector2(.96f,.92f), () => StartCoroutine(Save()));
    ...
    private void CharacterDetail(OwnedCharacter owned)
    {
        ...
        Button(page, "이 고양이 선택 · 저장", new Vector2(0,.09f), new Vector2(1,.17f), () => StartCoroutine(SelectCharacter(owned)));
        var level = Button(page, "레벨업 · 성장 규칙 준비 중", new Vector2(0,0), new Vector2(1,.07f), () => {});
        level.interactable = false;
    }

    private IEnumerator SelectCharacter(OwnedCharacter owned)
    {
        if (saving || !data.characters.Contains(owned)) yield break;
        string previous = data.loadout.characterId;
        data.loadout.characterId = owned.characterId;
        yield return Save(() => data.loadout.characterId = previous);
    }
    ...
    private IEnumerator Save(Action rollback = null)
    {
        if (saving) yield break;
        if (preview) { status.text = "미리보기 변경입니다. 서버에는 저장하지 않습니다."; yield break; }
        saving = true;
        status.text = "저장 중…";
        yield return null; yield return null;
        bool success = false;
        try { success = BackendGameData.Instance.GameDataUpdate(); }
        finally { saving = false; if (!success) rollback?.Invoke(); }
        status.text = success ? "저장했습니다." : "저장하지 못했습니다. 다시 시도해주세요.";
    }
    ...
    private void OnDestroy() { if (ownCatalog && catalog != null) Destroy(catalog); }
}
